...
async def on_ready() -> None:
    response_to_ready = ServerEvents.return_on_ready(bot, GUILD, CHANNEL_ID)
    await response_to_ready
    await bot.process_bot(bot, GUILD)


@bot.event
async def on_member_join(member) -> None:
    response_to_joining = ServerEvents.return_on_joining(member, CHANNEL_ID)
    await response_to_joining
    await bot.process_user(member, GUILD)


@bot.event
async def on_member_remove(member) -> None:
    response_to_removing = ServerEvents.return_on_removing(member, CHANNEL_ID)
...
async def on_member_update(before, after) -> None:
    response_to_updating = ServerEvents.return_on_updating(before, CHANNEL_ID)
    await response_to_updating
    await bot.process_user(before, after, GUILD)


@bot.event
async def on_member_ban(guild, user) -> None:
    response_to_banning = ServerEvents.return_on_banning(guild, user, CHANNEL_ID)
...
async def on_member_unban(guild, user) -> None:
    response_to_unbanning = ServerEvents.return_on_unbanning(guild, user, CHANNEL_ID)
    await response_to_unbanning
    await bot.process_unban(user, guild)
...
async def on_message_edit(
    sent_message: discord.Message, edited_message: discord.Message
) -> None:
    user = sent_message.author
    response_to_edditing = ServerEvents.return_on_editing(
        sent_message, edited_message, user
    )
    await response_to_edditing
    await bot.edit_message(sent_message, edited_message)
...
async def users(ctx) -> None:
    users_list = UserCommands.list_of_users(ctx, GUILD, bot)
