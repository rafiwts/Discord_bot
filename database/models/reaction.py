...
class Reaction(DefaultDatabaseModel):
    user_from: int = peewee.ForeignKeyField(
        DiscordUser, backref="senders", on_delete="CASCADE"
    )
    user_to: int = peewee.ForeignKeyField(
        DiscordUser, backref="recipients", on_delete="CASCADE"
    )
    message: int = peewee.ForeignKeyField(
        Message, backref="reactions", on_delete="CASCADE"
    )
    ...
    def create_new_reaction(
        cls, user_from: peewee.ForeignKeyField, current_message: discord.Message
    ):
        return cls.create(
            user_from=user_from,
