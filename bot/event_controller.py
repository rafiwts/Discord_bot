...
class Controller:
    ...
    async def message_create_controller(self, sent_message: discord.Message) -> None:
        ...
        new_message = Message.create_new_message(
            sent_message=sent_message, user=discord_user.id
        )

        new_message.save()

    async def message_edit_cotroller(
        self, sent_message: discord.Message, edited_message: discord.Message
    ) -> None:
        Message.edit_message(discord_id=sent_message.id, edited_message=edited_message)
    ...
    async def event_controller(self, sent_message: discord.Message) -> None:
        ...
        if current_event is None:
            new_event = Event.create_new_event(
                user=discord_user.id, sent_message=sent_message
            )

            new_event.save()
        else:
            update_event = Event.increment_counter(
    ...
    async def reaction_controller(
        ...
        if reaction.event_type == "REACTION_ADD":
            reaction_for_existing_message = Reaction.get_or_none(
                user_from=user.id,
                user_to=current_message.user.id,
                message=current_message.id,
            )
            if reaction_for_existing_message is None:
                reaction_to_message = Reaction.create_new_reaction(
                    user_from=user.id, current_message=current_message
                )

                reaction_to_message.save()

                Message.edit_message(
                    discord_id=reaction.message_id,
                    reaction_counter=Message.reaction_counter + 1,
                )
            else:
                Reaction.update(edited_at=datetime.now()).where(
                    (Reaction.user_to == current_message.user.id)
                    & (Reaction.message == current_message.id)
                ).execute()
        else:
            reaction_to_message = Reaction.get(
                user_from=user.id,
                user_to=current_message.user.id,
                message=current_message.id,
            )
    ...
    async def command_controller(self, message: discord.Message) -> None:
        ...
        new_command = Command.get_or_none(
            (Command.content == message.content) & (Command.user == discord_user.id)
        )

        if new_command is None:
            new_command = Command.create_new_command(
                message=message, user=discord_user.id
            )

            new_command.save()
        else:
            update_counter = Command.increment_counter(
        ...
        new_message = Message.create_new_message(
            sent_message=message, user=discord_user.id, command=new_command.id
        )
    ...
    async def bot_controller(self, bot: commands.Bot, guild: str) -> None:
        ...
        if admin_user is None:
            new_admin_user = DiscordUser.create_new_admin(
                bot_owner=bot.application.owner, guildname=guild
            )
        ...
        try:
            new_discord_bot = BotUser.create_new_bot(
                bot=bot, owner_id=new_admin_user.id
            )
    ...
    async def add_or_delete_user_controller(
        self, user: discord.Member, guild: str
    ) -> None:
        existing_discord_user = DiscordUser.get_or_none(
            DiscordUser.discord_id == user.id
        )
        if existing_discord_user is None:
            new_discord_user = DiscordUser.create_new_user(
                discord_user=user, guildname=guild
            )
    ...
    async def update_user_controller(
        self, user: discord.Member, guild: discord.Guild, updated_user: discord.Member
    ):
        DiscordUser.update_user_data(
            discord_user=user, updated_user=updated_user, guild=guild
        )

    async def ban_user_controller(
        self, member: discord.Member, guild: discord.Guild
    ) -> None:
        DiscordUser.update_user_data(discord_user=member, guild=guild, banned=True)

    async def unban_user_controller(
        self, member: discord.Member, guild: discord.Guild
    ) -> None:
        DiscordUser.update_user_data(discord_user=member, guild=guild, banned=False)
