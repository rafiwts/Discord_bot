...
class Event(DefaultDatabaseModel):
    ...
    @classmethod
    def create_new_event(cls, user: peewee.ForeignKeyField, 
                              sent_message: discord.Message = None):
        return cls.create(name=sent_message.content,
                          cteated_at=sent_message.created_at,
                          user=user)
            
    @classmethod
    def increment_counter(cls, user: discord.Member,
                               sent_message: discord.Message,
                               counter: peewee.IntegerField):
        return cls.update(counter=counter)\
                  .where((cls.name==sent_message.content) &
                         (cls.user==user))
